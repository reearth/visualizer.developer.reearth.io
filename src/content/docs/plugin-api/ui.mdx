...
```ts
reearth.camera.on<T extends keyof UIEventType>(
  type: T,
...
- once: boolean: If `true`, the listener will automatically be removed after it is invoked once. Defaults to `false`.
