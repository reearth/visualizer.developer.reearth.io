...
The show method displays a popup with custom HTML content at a specified position in the **reearth** interface. The popup’s appearance, location, and offset can be configured, making it flexible for various UI needs.
...
This enables sending messages between the popup window and other components within **`reearth`**. This function is useful for communication in scenarios where a popup needs to relay information back to the application, trigger actions in other components, or respond to user interactions within the popup content.
...
This allows users to modify the appearance and position of an existing popup in **`reearth`**. Users can dynamically adjust properties such as the popup’s width, height, position, and offset without needing to recreate the popup.
...
The PopupPosition type controls where the popup appears relative to the target element. It provides a range of options for positioning the popup above, below, or to the sides of the target, with additional options for alignment adjustments (start, center, or end).
...
The PopupOffset type allows users to control the popup’s distance from the target. It provides two options: A number that adjusts the offset along the main axis, and an object for more detailed control over different axes, including main, cross, and alignment axes.
...
**`alignmentAxis?: number | null`**: Provides fine-tuned control over alignment along a secondary axis, useful for more complex placements. Setting alignmentAxis to null disables adjustments along this axis.
