...
import DeleteLayerCode from "@code/plugin-api/layers/findbyid-layer?raw";
...
```ts
reearth.layers.override: (id: string, layer: Partial<Layer> & { property?: unknown } ) => void;
```
...
layerId: The unique identifier of the layer to be selected.
featureId: An array of string parameters, where each string is the ID of a feature
