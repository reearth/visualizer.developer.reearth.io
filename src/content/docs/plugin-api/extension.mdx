...
```ts
reearth.extension.postMessage(id: string, message: unknown) => void;
```
...
The message event triggers whenever a generic message is sent to the extension. This event is used to listen for messages from other components, such as widgets or blocks, within the plugin.
...
- `"block"`: A generic block extension.
- `"infoboxBlock"`: An infobox block extension.
- `"storyBlock"`: A story block extension.

**`runTimes: number | undefined`**: The number of times this extension instance has been executed.

- `number`: Indicates how many times the instance has been run.
- `undefined`: This value is unavailable or not applicable.
